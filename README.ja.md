...
> **重要**: SAM 3 を使用する前に、[SAM 3 Hugging Face リポジトリ](https://huggingface.co/facebook/sam3)でチェックポイントへのアクセスを申請してください。承認後、チェックポイントを自動ダウンロードするには認証が必要です（例: `huggingface-cli login`）。
>
> または、`sam3.pt` を手動でダウンロードし、プラグインのチェックポイント欄にローカルパスを指定することも可能です。
